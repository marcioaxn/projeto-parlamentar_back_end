...
// Início dos controllers específicos do estado
use App\Http\Controllers\TabIndicadoresEstadosController;
// Fim dos controllers específicos do estado
...
use App\Http\Controllers\FundosController;
...
class MunicipiosController extends Controller
{
    ...
    public function instanciarFundosController()
    {
        return new FundosController;
    }
    ...
    public function index($sglUf = null, $codNomMunicipio = null)
    {
        ...
        // Início da parte de instanciar os Controllers necessários para o estado
        $tabIndicadoresEstados = $this->instanciarTabIndicadoresEstadosController();
        // Fim da parte de instanciar os Controllers necessários para o estado
        ...
        $tabNovoPac = $this->instanciarTabNovoPacController();
        $fundos = $this->instanciarFundosController();
        ...
        $arrayAreasInvestimento = [];
        $resumoTciMunicipio = [];
        $resumoTciEstado = [];
        $areasInvestimento = [];
        $estruturaTableParaEditar = [];
        $novosPac = [];
        $colunasVisiveisNovoPac = [];
        $pac = [];
        $resumoTci = [];
        $tciMunicipio = [];
        $tci = [];
        $sglUfNomMunicipio = null;
        $getGrupoReconhecimentosPorCodIbgeEstadoPorTempo = [];
        $getReconhecimentosSedecPorCodIbge = [];
        $getPisf = [];
        $fundoResumo = [];
        $fundosResumo = [];
        $fundoResumoMunicipio = null;

        $s2id = $this->instanciarS2idController();

        $areasInvestimento = $visTciEmendas->getGrupoAreasInvestimento();

        foreach ($areasInvestimento as $areaInvestimento) {
            $arrayAreasInvestimento[$areaInvestimento->dsc_area_investimento] = $areaInvestimento->dsc_area_investimento;
        }
        ...
        if (isset($sglUf) && !is_null($sglUf) && $sglUf != '') {
            ...
            if (isset($codNomMunicipio) && !is_null($codNomMunicipio) && $codNomMunicipio != '') {
                ...
                // Início da parte para pegar os dados do PAC que contêm no campo sgl_uf a $sglUf
                $colunasVisiveisNovoPac = ['nom_empreendimento_divulgacao', 'nom_municipio', 'codigoUnidade', 'dsc_situacao', 'dsc_fase', 'prc_execucao_fisica', 'updated_at'];
                $estruturaTableParaEditar = $tabNovoPac->estruturaTableParaEditar();
                $novosPac = $tabNovoPac->getDadosPacPorCodMunicipio($codMunicipio);
                // Fim da parte para pegar os dados do PAC que contêm no campo sgl_uf a $sglUf
                ...
                $resumoTciMunicipio = $visTciEmendas->getResumoMunicipio($codMunicipio);

                $getReconhecimentosSedecPorCodIbge = $s2id->getGrupoReconhecimentosPorCodIbgePorTempo($codMunicipio);
                ...
                $tciMunicipio = $visTciEmendas->getTCIMunicipio($codMunicipio);

                // Início da parte para pegar os dados resumidos do fundo
                $fundoResumoMunicipio = $fundos->getFundosResumoValoresMunicipio($sglUf, $codMunicipio);
                $fundosResumo = $fundos->getFundosResumoValoresMunicipios($sglUf, $codMunicipio);
                // Fim da parte para pegar os dados resumidos do fundo

            }
            // Fim da verificação se a variável $codNomMunicipio contém algo
            // Início da parte para pegar os dados relativos ao estado
            else {
                $indicadoresEstado = $tabIndicadoresEstados->getIndicadoresEstado($sglUf);

                $resumoTciEstado = $visTciEmendas->getResumoEstado($sglUf);

                $codIbgeEstado = $indicadoresEstado->cod_ibge;

                $getGrupoReconhecimentosPorCodIbgeEstadoPorTempo = $s2id->getGrupoReconhecimentosPorCodIbgeEstadoPorTempo($codIbgeEstado);

                // Início da parte para pegar as rotas
                $rotasEstado = $tabRotas->getRotasPorEstado($codIbgeEstado);
                // Fim da parte para pegar as rotas

                // Início da parte para pegar os dados do TSE por tipo de parlamentar
                $tseGovernador = $tabTseConsolidada->getGovernadorPorCodMunicipio($sglUf);
                $tseSenadores = $tabTseConsolidada->getSenadoresPorEstado($sglUf);
                $tseDeputadosFederais = $tabTseConsolidada->getDeputadosFederaisPorEstado($sglUf);
                // Fim da parte para pegar os dados do TSE por tipo de parlamentar

                // Início da parte para pegar os dados do PAC que contêm no campo sgl_uf a $sglUf
                $colunasVisiveisNovoPac = ['nom_empreendimento_divulgacao', 'nom_municipio', 'codigoUnidade', 'dsc_situacao', 'dsc_fase', 'prc_execucao_fisica', 'updated_at'];
                $estruturaTableParaEditar = $tabNovoPac->estruturaTableParaEditar();
                $novosPac = $tabNovoPac->getDadosPacPorSglUf($sglUf);
                $pac = $tabNovoPac->getDadosPacPorIbge($sglUf);
                // Fim da parte para pegar os dados do PAC que contêm no campo sgl_uf a $sglUf

                // Início da parte para pegar os dados da TCI que contêm no campo uf a $sglUf
                $tciMunicipio = $visTciEmendas->getTCIEstado($sglUf);
                // Fim da parte para pegar os dados da TCI que contêm no campo uf a $sglUf

                // Início da parte para pegar os dados resumidos do fundo
                $fundoResumo = $fundos->getFundosResumoValores($sglUf);
                $fundosResumo = $fundos->getFundosResumoValoresMunicipios($sglUf);
                // Fim da parte para pegar os dados resumidos do fundo

            }
            // Fim da parte para pegar os dados relativos ao estado
        ...
        $temas = ['Empreendimentos do novo PAC', 'TSE', 'Carteira de Investimento', 'Fundos de Desenvolvimento Regional'];

        // Verificação se existe o conteúdo do PAC e se caso não existir o tema PAC é retirado, para evitar tema desnecessário
        /*
        if (is_iterable($pac) && $pac->count() <= 0) {
            foreach (array_keys($temas, 'Empreendimentos do novo PAC', true) as $key) {
                unset($temas[$key]);
            }
        }
        */
        ...
        return view('estadoMunicipio.index', ['sgl_uf' => $sglUf, 'cod_nom_municipio' => $codNomMunicipio])
            ...
            ->with('areasInvestimento', $visTciEmendas->getGrupoAreasInvestimento())
            ->with('arrayAreasInvestimento', $arrayAreasInvestimento)
            ->with('pac', $pac)
            ->with('colunasVisiveisNovoPac', $colunasVisiveisNovoPac)
            ->with('estruturaTableParaEditar', $estruturaTableParaEditar)
            ->with('novosPac', $novosPac)
            ->with('resumoTciEstado', $resumoTciEstado)
            ->with('resumoTciMunicipio', $resumoTciMunicipio)
            ->with('tciMunicipio', $tciMunicipio)
            ->with('tci', $tci)
            ->with('sglUfNomMunicipio', $sglUfNomMunicipio)
            ->with('getGrupoReconhecimentosPorCodIbgeEstadoPorTempo', $getGrupoReconhecimentosPorCodIbgeEstadoPorTempo)
            ->with('getReconhecimentosSedecPorCodIbge', $getReconhecimentosSedecPorCodIbge)
            ->with('getPisf', $getPisf)
            ->with('fundoResumo', $fundoResumo)
            ->with('fundosResumo', $fundosResumo)
            ->with('fundoResumoMunicipio', $fundoResumoMunicipio);
    }
