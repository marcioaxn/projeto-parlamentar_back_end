...
class PromptController extends Controller
{
    ...
    public function getResumoExecutivoParlamentar($codParlamentar = null, $nomeParlamentar = null, $cargoParlamentar = null, $sglPartido = null, $sglUfRepresentacao = null)
    {
        if (!empty($nomeParlamentar) && !empty($cargoParlamentar) && !empty($sglPartido) && !empty($sglUfRepresentacao)) {

            $cacheKey = "resumo_executivo_{$codParlamentar}";

            Cache::forget($cacheKey);

            return Cache::remember($cacheKey, now()->addHours(24), function () use ($nomeParlamentar, $cargoParlamentar, $sglPartido, $sglUfRepresentacao) {
                $response = OpenAI::chat()->create([
                    'model' => 'gpt-3.5-turbo',
                    'messages' => [
                        ['role' => 'system', 'content' => 'Você é Especialista em Relações Governamentais (Lobbyist), Lobbyista sênior que trabalha com a formulação e alteração de políticas públicas de forma mais direta e com mais poder, buscando influenciar a legislação de acordo com interesses específicos e interagindo com vários stakeholders em nível executivo e legislativo. Cada tema deve trazer novas abordagens diariamente para evitar repetições e nunca deixe no texto X%, sempre mostre o número, pode citar o percentual ou valor, não se for citar mostre o percentual ou valor e nunca deixe o texto X%. Os dados devem ser precisos, com fontes oficiais do ano corrente, se indisponíveis, do ano anterior, e impacto direto para o parlamentar. O Resumo executivo deve ser rico em detalhes, nada genérico, e as oportunidades e ações sugeridas devem ser bem especificas, de forma que o parlamentar não tenha dúvidas sobre a informação passada. Não inclua a data no título do resumo executivo.'],
                        [
                            'role' => 'user',
