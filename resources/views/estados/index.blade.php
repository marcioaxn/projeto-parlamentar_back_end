@if (!empty($tse))
    @php
        if (isset($getParlamentar) && $getParlamentar) {
            isset($getParlamentar->num_total_votos) && !is_null($getParlamentar->num_total_votos) && $getParlamentar->num_total_votos != '' ? ($totalVotos = $getParlamentar->num_total_votos) : ($totalVotos = 0);
        } else {
            $totalVotos = 0;
        }
    @endphp
    <div class="row pl-1">
        ...
        <div class="col-xs-12 col-sm-12 col-md-12 col-lg-12 mb-1 text-left">
            <div for="" class=" <?php isset($getParlamentar) && $getParlamentar->dsc_casa === 'Câmara dos Deputados' ? print 'bg-camara-sub-titulo-modal' : print 'bg-senado-sub-titulo-modal'; ?> rounded mt-2 pt-1 pb-1 pl-2">Principais Indicadores municipais
                    ...
                    @foreach ($tse as $key => $value)
                        <tr>
                            ...
                            <td style="width: 45%!Important;">
                                @php
                                    isset($value->nm_mun) && !is_null($value->nm_mun) && $value->nm_mun != '' ? ($municipioUrl = $value->cd_mun) : ($municipioUrl = '');
                                    isset($value->nm_mun) && !is_null($value->nm_mun) && $value->nm_mun != '' ? ($municipio = '/' . tirarAcentuacao(passarTextoParaMaiusculo($value->nm_mun))) : ($municipio = '');
                                @endphp
                                <a href="{!! route('municipio', [$value->sigla_uf, $municipioUrl]) !!}" target="_blank">
                                    {!! $value->sigla_uf . $municipio !!}
                                </a>
                            </td>
                            <td class="text-right font-numero">
                                @php
                                    $value->qt_votos_nominais > 0 && $totalVotos > 0 ? ($prcVotos = ($value->qt_votos_nominais / $totalVotos) * 100) : ($prcVotos = 0);
                                @endphp
