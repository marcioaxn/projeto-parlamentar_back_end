...
@section('content')
    ...
    <div class="">
        ...
        <form action="{{ route('contratos.update', $contrato->cod_contrato) }}" method="POST">
            @csrf
            @method('PUT')
            ...
            <div class="row g-3">
                    ...
                    <select name="cod_gabinete" id="cod_gabinete" class="form-select">
                        ...
                        @foreach ($gabinetes as $gabinete)
                            <option value="{{ $gabinete->cod_gabinete }}"
                                {{ old('cod_gabinete', $contrato->cod_gabinete) == $gabinete->cod_gabinete ? 'selected' : '' }}>
                    ...
                    <select name="cod_plano" id="cod_plano" class="form-select">
                        ...
                        @foreach ($planos as $plano)
                            <option value="{{ $plano->cod_plano }}"
                                {{ old('cod_plano', $contrato->cod_plano) == $plano->cod_plano ? 'selected' : '' }}>
                                {{ $plano->nom_plano }}
                            </option>
                        @endforeach
                    </select>
                ...
                <div class="col-md-2">
                    ...
                    <input type="date" name="dat_inicio" id="dat_inicio" class="form-control"
                        value="{{ old('dat_inicio', $contrato->dat_inicio ? $contrato->dat_inicio->format('Y-m-d') : '') }}">
                </div>

                <div class="col-md-2">
                    <label for="dat_fim" class="form-label"><span class="text-bold font-numero">4.</span> Data Fim</label>
                    <input type="date" name="dat_fim" id="dat_fim" class="form-control"
                        value="{{ old('dat_fim', $contrato->dat_fim ? $contrato->dat_fim->format('Y-m-d') : '') }}">
                </div>
                ...
                <div class="col-md-3">
                    ...
                    <div class="input-group">
                        ...
                        <input type="text" name="val_total" id="val_total"
                            ...
                            value="{{ old('val_total', number_format($contrato->val_total, 2, ',', '.')) }}"
                            step="0.01">
                    </div>
                </div>
                ...
                <div class="col-md-2">
                    ...
                    <div class="input-group">
                        ...
                        <input type="text" name="val_desconto_aplicado" id="val_desconto_aplicado"
                            class="form-control text-right mascara-dinheiro font-numero"
                            value="{{ old('val_desconto_aplicado', number_format($contrato->val_desconto_aplicado ?? 0, 2, ',', '.')) }}"
                            step="0.01">
                    </div>
                </div>
                ...
                <div class="col-md-3">
                    ...
                    <div class="input-group">
                        ...
                        <input type="text" name="val_sub_total" id="val_sub_total"
                            class="form-control text-right mascara-dinheiro font-numero"
                            value="{{ old('val_sub_total', number_format($contrato->val_sub_total, 2, ',', '.')) }}"
                            readonly>
                    </div>
                </div>
                ...
                <div class="col-md-9">
                    <label for="dsc_observacoes" class="form-label"><span class="text-bold font-numero">8.</span>
                        Observações</label>
                    <textarea name="dsc_observacoes" id="dsc_observacoes" class="form-control" rows="3">{{ old('dsc_observacoes', $contrato->dsc_observacoes) }}</textarea>
                </div>
                ...
                <div class="col-md-3">
                    ...
                    <select name="sta_ativo" id="sta_ativo" class="form-select">
                        <option value="A" {{ old('sta_ativo', $contrato->sta_ativo) == 'A' ? 'selected' : '' }}>Ativo
                        </option>
                        <option value="I" {{ old('sta_ativo', $contrato->sta_ativo) == 'I' ? 'selected' : '' }}>
                            Inativo</option>
                    </select>
                </div>
    ...
    <script>
        ...
        window.addEventListener('load', function() {
            const valTotal = document.getElementById('val_total').value;
            const valDescontoInput = document.getElementById('val_desconto_aplicado');

            // Habilita o campo de desconto se houver valor total
            if (valTotal && valTotal !== '0,00') {
                valDescontoInput.disabled = false;
            }

            // Aplica todas as configurações do create
            setupAutoNavigation();
            setupDateValidation();
            setupValTotal();
